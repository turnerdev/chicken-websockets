(module websockets
  ...
  (import scheme (chicken base) (chicken blob) (chicken bitwise) (chicken io) (chicken process-context posix) (chicken foreign)
	  srfi-13 srfi-18 srfi-14 (chicken condition))
  (import srfi-1 srfi-4 spiffy intarweb uri-common base64 simple-sha1
	  mailbox comparse)
  ...
  (define close-timeout (make-parameter 50)) ;; TODO was 
...
") len s)))
  ...
  (define (close-code->u8vector code)
    (u8vector
     (arithmetic-shift (bitwise-and code 65280) -8)
     (bitwise-and code 255)))
	    ...
	    (lambda ()
		  ...
		  (begin
		    ...
		    (send-frame ws 'connection-close
				(close-code->u8vector (close-reason->close-code close-reason))
				#t)
		    (let loop ()
